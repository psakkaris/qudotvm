...
namespace qudot {
    void EPRUnit::mergeWorlds(QuMvN* qumvn, double epsilon) {
        ...
        concurrent_unordered_map<string, concurrent_unordered_set<size_t>> world_sigs;  
        ...
        tbb::parallel_for(world_sigs.range(), [&](const concurrent_unordered_map<string, concurrent_unordered_set<size_t>>::range_type &r) {
            for (auto it=r.begin(); it != r.end(); ++it) {
                if (it->second.size() > 1) {
